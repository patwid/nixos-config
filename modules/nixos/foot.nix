...
let
  ...
  colors' = colors |> builtins.mapAttrs (_: lib.strings.removePrefix "#");
in
{
  imports = [ inputs.nix-wrapper-modules.nixosModules.foot ];
  ...
  config = {
    wrappers.foot = {
      enable = true;
      settings = {
        main.font = "monospace:size=${toString config.terminal.fontsize}";
        main.pad = "8x4";
